...
export default function CustomerTracking() {
  ...
  return (
    <div className="space-y-6" id="customer-tracking-root">
      ...
      {/* Hero Header */}
      <div className="bg-[#0F172A] p-6 md:p-8 rounded-2xl text-white relative overflow-hidden shadow-lg border border-slate-800">
        <div className="absolute top-0 right-0 transform translate-x-12 -translate-y-12 w-64 h-64 bg-sky-500/5 rounded-full shrink-0"></div>
        <div className="relative z-10 max-w-2xl">
          <div className="inline-block bg-sky-500/15 text-sky-400 text-[10px] font-bold tracking-tight uppercase px-2.5 py-0.5 rounded border border-sky-500/20 mb-2">
            Masyarakat Ceria Tanpa Login
          </div>
          <h2 className="text-xl md:text-2xl font-black text-slate-100">Lacak Cucian Real-Time Laundry Kita</h2>
          <p className="text-xs text-slate-300 mt-1.5 max-w-md leading-relaxed">
            Cukup masukkan nomor ponsel Anda untuk memantau status cuci, saldo deposit, historis, serta estimasi cucian selesai secara langsung.
          </p>
        </div>
      </div>
                ...
                activeOrders.map(order => {
                  ...
                  return (
                    <div key={order.id} className="p-5 bg-slate-50 border border-slate-200 rounded-2xl space-y-4 shadow-sm">
                      ...
                      {/* Items lists summary */}
                      <div className="p-3 bg-white rounded-xl border border-slate-200 text-[11px] font-mono leading-relaxed text-slate-600">
                        🛒 Item laundry: {order.items.map(it => `${it.serviceName} (qty: ${it.quantity})`).join(', ')}
                      </div>
